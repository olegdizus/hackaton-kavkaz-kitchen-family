	...
	function prepareYAxis (g, render) {
       /* if (!_chart.isOrdinal()) {
            ...
            if (_chart.elasticY() || _y.domain().length === 0) {
                _y.domain(_chart._ordinalXDomain());
            }
        }*/

        // has the domain changed?
        var xdom = _y.domain();
        if (render || compareDomains(_lastYDomain, xdom)) {
            _chart.rescale();
        }
        _lastYDomain = xdom;
        ...
        // please can't we always use rangeBands for bar charts?
        if (_chart.isOrdinal()) {
            _y.rangeBands([0, _chart.yAxisLength()], _rangeBandPadding,
                          _chart._useOuterPadding() ? _outerRangeBandPadding : 0);
        } else {
            _y.range([_chart.yAxisLength(), 0]);
        }

        _yAxis = _yAxis.scale(_chart.y());

        //_chart._renderHorizontalGridLinesForAxis(g, _chart.y(), _chart.yAxis())
        ...
        if (_renderHorizontalGridLine) {
            if (gridLineG.empty()) {
                gridLineG = g.insert('g', ':first-child')
                    ...
                    .attr('transform', 'translate(' + _chart.margins().right + ',' + _chart.margins().bottom + ')');//TODO: есть сомнения насчет right and down
            }

            var ticks = _yAxis.tickValues() ? _yAxis.tickValues() :
                    (typeof _y.ticks === 'function' ? _y.ticks(_yAxis.ticks()[0]) : _y.domain());
            ...
            var linesGEnter = lines.enter()
                ...
                .attr('x1', _chart._yAxisX() - _chart.margins().right)
                .attr('y2', function (d) {
                    return _y(d);
                })
                .attr('x2', 0)
